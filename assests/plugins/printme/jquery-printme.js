...
jQuery.fn.printMe = function (options) {
  ...
  return this.each(function () {
    ...
    for (i in settings.path) {
      w.document.write(
        '<link rel="stylesheet" href="' + settings.path[i] + '">',
      );
    }
    ...
    w.document.write(
      '<script type="text/javascript">document.fonts.ready.then(function(){ setTimeout(function(){ window.print(); window.close(); }, 500); });</script></body></html>',
    );
    w.document.close();
  });
};
