...
// Modify WHERE clause to only show active products
$sql = "SELECT product.product_id, product.product_name, product.product_image, product.brand_id,
    ...
    INNER JOIN brands ON product.brand_id = brands.brand_id 
    INNER JOIN categories ON product.categories_id = categories.categories_id  
    WHERE product.status = 1 AND product.active = 1 ".$searchQuery." 
    ORDER BY ".$columnName." ".$columnSortOrder." 
    LIMIT ".$row.", ".$rowperpage;

// Also update the count queries
$totalRecords = $connect->query("SELECT COUNT(*) as allcount FROM product WHERE status = 1 AND active = 1")->fetch_assoc();
$totalRecords = $totalRecords['allcount'];

$totalRecordwithFilter = $connect->query("SELECT COUNT(*) as allcount FROM product 
    INNER JOIN brands ON product.brand_id = brands.brand_id 
    INNER JOIN categories ON product.categories_id = categories.categories_id  
    WHERE product.status = 1 AND product.active = 1 ".$searchQuery)->fetch_assoc();
$totalRecordwithFilter = $totalRecordwithFilter['allcount'];
...
while($row = $result->fetch_assoc()) {
    ...
    if($_SESSION['Status']=='5'){
        $button .= '<li class="disabled"><a href="javascript:void(0);" style="cursor: not-allowed; opacity: 0.5;">
    ...
    // Data array
    $data[] = array(
        "product_image" => "<div class='text-center'>".$productImage."</div>",
        "product_id" => "<div class='text-center'>".$row['product_id']."</div>",
        "brand_name" => "<div class='text-center'>".$row['brand_name']."</div>",
        "product_name" => "<div class='text-center'>".$row['product_name']."</div>",
        "categories_name" => "<div class='text-center'>".$row['categories_name']."</div>",
        "features" => "<div class='text-center'>".$features."</div>",
        "brate" => "<div class='text-center'>".$row['brate']."</div>",
        "rate" => "<div class='text-center'>".$row['rate']."</div>",
        "quantity" => "<div class='text-center'>".$row['quantity']."</div>",
        "clor" => "<div class='text-center'>".$row['clor']."</div>",
        "active" => $active,
        "button" => $button
    );
}
