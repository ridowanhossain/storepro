...
if ($isDataTablesRequest) {
    ...
    // Default values for regular AJAX request
    $draw = 1;
    $row = 0;
    $rowperpage = 10;
    $columnIndex = 0;
...
$sql = "SELECT orders.order_id, orders.order_date, orders.client_name, 
        ...
        users.full_name, orders.o_feature, orders.due 
    FROM orders 
    INNER JOIN users ON users.user_id = orders.s_name 
...
while($row = $result->fetch_array()) {
    ...
    if($_SESSION['Status']=='5' || $_SESSION['Status']=='2'){
        $button .= '<li><a href="quick-order-edit='.$orderId.'" id="editOrderModalBtn"> <i class="glyphicon glyphicon-edit"></i> সম্পাদনা</a></li>';
    }
    $button .='<li><a type="button" data-toggle="modal" id="paymentOrderModalBtn" data-target="#paymentOrderModal" onclick="paymentOrder('.$orderId.')"> <i class="glyphicon glyphicon-save"></i> পরিশোধ</a></li>
        <li><a type="button" href="quick-order-view='.$orderId.'" target="_blank"> <i class="fa fa-file-text"></i> প্রতিবেদন</a></li>
    ...
    $data[] = array(
        ...
        "5" => $row[5],
        "6" => $row[7],
        "7" => number_format($row['due'], 2) . ' ৳', // Add due amount
        "8" => $paymentStatus,
        "9" => $row[6],
        "10" => $button
    );
