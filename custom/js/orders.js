...
$(document).ready(function () {
  ...
  if (divRequest == "add") {
                ...
                if (response.success == true) {
                  ...
                  $(".success-messages").html(
                    '<div class="alert alert-success">' +
                      ...
                      '<a href="order.php?o=add" class="btn btn-default" style="margin-left:10px;"> <i class="glyphicon glyphicon-plus-sign"></i> অর্ডার করুন </a>' +
                      '<a href="regular-order.php?o=add" class="btn btn-default" style="margin-left:10px;"> <i class="glyphicon glyphicon-plus-sign"></i> নিয়মিত অর্ডার করুন </a>' +
                      "</div>"
                  );
  ...
  } else if (divRequest == "show") {
    // top nav child bar
    $("#topNavManageOrder").addClass("active");

    manageOrderTable = $("#rmanageOrderTable").DataTable({
      processing: true,
      serverSide: true,
      responsive: true, // Add responsive feature
      ajax: {
        url: "php_action/rfetchOrder.php",
        type: "POST",
    ...
    $("#editOrderForm")
      ...
      .bind("submit", function () {
        ...
        // Remove validation for payment type and status since they're disabled
        // var paymentType = $("#paymentType").val();
        // var paymentStatus = $("#paymentStatus").val();
        ...
        if (
          ...
          paid &&
          discount
          // Remove from validation check since they're disabled
          // && paymentType &&
          // paymentStatus
        ) {
          if (validateProduct == true && validateQuantity == true) {
...
function paymentOrder(orderId = null) {
  if (orderId) {
    ...
    $("#updatePaymentOrderBtn").button("reset").unbind("click");
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        $("#sr_id").val(response.order[13]);

        var paidAmount = response.order[9];
        var dueAmount = response.order[10];
        var grandTotal = response.order[8];

        // Add this function to calculate remaining due
        function calculateRemainingDue() {
          var dueAmount = parseFloat($("#due").val()) || 0;
          var payAmount = parseFloat($("#payAmount").val()) || 0;
          var remainingDue = dueAmount - payAmount;

          // Set the remaining due amount
          $("#remainingDue").val(remainingDue.toFixed(2));

          // Auto set payment status based on remaining due
          if (remainingDue <= 0) {
            $("#paymentStatus").val("1"); // পরিশোধ
          } else {
            $("#paymentStatus").val("3"); // বাঁকি
          }
        }

        // Also update the payAmount input event to use the new function
        $("#payAmount").on("input", function () {
          calculateRemainingDue();
        });

        // update payment
        $("#updatePaymentOrderBtn").on("click", function () {
...
$(document).ready(function () {
  $("#nmbr").change(function () {
    var division_id = $(this).val();
    var order_id = $("#orderId").val(); // Get the order ID from hidden field

    if (division_id) {
      $.ajax({
        url: "php_action/fetchdistrict2.php",
        method: "POST",
        dataType: "json",
        data: {
