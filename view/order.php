...
<ol class="breadcrumb">
	...
	<li class="active">
		...
		<?php } else if ($_GET['o'] == 'manord') { ?>
			দ্রুত অর্ডার পরিচালনা
		<?php } else if ($_GET['o'] == 'editOrd') { ?>
			দ্রুত অর্ডার সম্পাদনা
	...
	<div class="main-table">
		...
		<div class="product-table-card">
			<div class="table-title">
				<i class="glyphicon glyphicon-plus-sign"></i> দ্রুত অর্ডার
			</div>
			...
			<form class="form-horizontal" method="POST" action="php_action/createOrder.php" id="createOrderForm">
				<input type="hidden" class="form-control" id="sr_id" name="sr_id" value="0" autocomplete="off" />
				...
				<div class="col-sm-5">
					<div class="form-group order-group">
						...
						<input type="text" class="form-control" id="clientName" name="clientName" placeholder="ক্রেতার নাম" autocomplete="off" />
					</div>
				</div> <!--/form-group-->
				<div class="col-sm-5 col-sm-offset-1">
					<div class="form-group order-group">
						<label for="clientContact" class="control-label">ক্রেতার মোবাইল নাম্বার</label>
						<input type="text" class="form-control" id="clientContact" name="clientContact" placeholder="ক্রেতার মোবাইল নাম্বার" autocomplete="off" />
					</div>
				</div> <!--/form-group-->
				...
				<div class="col-sm-5">
					<div class="form-group order-group">
						...
						<input type="text" class="form-control" id="clientaddress" name="clientaddress" placeholder="ক্রেতার ঠিকানা" autocomplete="off" />
					</div>
				</div> <!--/form-group-->
				...
				<div class="col-sm-10 col-sm-offset-1">
					<div class="form-group order-group">
						...
						<input type="text" class="form-control" id="o_feature" name="o_feature" value="N/A" placeholder="Note Well" />
					</div>
				</div> <!--/form-group-->
				...
				<table class="table table-bordered modern-table dataTable no-footer dtr-inline" id="productTable">
					<thead>
						<tr>
							...
							<th>পরিমাপ</th>
							<th>দাম</th>
							<th class="text-center add-th"><button type="button" class="btn btn-default add-r" onclick="addRow()" id="addRowBtn" data-loading-text="Loading..."> <i class="glyphicon glyphicon-plus-sign"></i> পণ্য যুক্ত করুন </button>
							</th>
						</tr>
					</thead>
					<tbody>
						...
						for ($x = 1; $x < 2; $x++) { ?>
							<tr id="row<?php echo $x; ?>" class="table-order <?php echo $arrayNumber; ?>">
								...
								<td>
									<div class="form-group">
										<input type="number" name="rate[]" id="rate<?php echo $x; ?>" autocomplete="off" class="form-control" step="0.01" min="0" onkeyup="syncRateValue(<?php echo $x; ?>); getTotal(<?php echo $x; ?>); updateOrderSummary();" onchange="syncRateValue(<?php echo $x; ?>); getTotal(<?php echo $x; ?>); updateOrderSummary();" />
								...
								<td>
									<div class="form-group">
										<input type="number" name="quantity[]" id="quantity<?php echo $x; ?>" oninput="getTotal(<?php echo $x ?>);" autocomplete="off" class="form-control" min="0" step="any" inputmode="decimal" pattern="^[0-9]*\.?[0-9]+$" />
									</div>
								</td>
								<td>
									<div class="form-group">
										<input type="text" name="clor[]" id="clor<?php echo $x; ?>" autocomplete="off" disabled="true" class="form-control" />
									</div>
								</td>
					...
					<div class="col-md-6">
						...
						<div class="form-group">
							...
							<div class="col-sm-9">
								...
								<input type="hidden" name="paymentStatus" value="3"> <!-- Add hidden input to ensure value is submitted -->
							</div>
						</div> <!--/form-group-->
				...
				<div class="form-group submitButtonFooter">
					<div class="col-sm-offset-2 col-sm-10">
						...
						<button type="submit" id="createOrderBtn" data-loading-text="Loading..." class="btn btn-success"><i class="glyphicon glyphicon-ok-sign"></i> সংরক্ষণ করুন</button>

						<button type="reset" class="btn btn-default" onclick="resetOrderForm()"><i class="glyphicon glyphicon-erase"></i> পুনঃস্থাপন</button>
					</div>
				</div>
			</form>
		<?php } else if ($_GET['o'] == 'manord') {
			...
			<div class="product-table-card">
				<div class="table-title">
					<?php if ($_GET['o'] == 'add') { ?>
						<i class="glyphicon glyphicon-plus-sign"></i> দ্রুত অর্ডার
					<?php } else if ($_GET['o'] == 'manord') { ?>
						<i class="glyphicon glyphicon-edit"></i> দ্রুত অর্ডার পরিচালনা
					<?php } else if ($_GET['o'] == 'editOrd') { ?>
						<i class="glyphicon glyphicon-edit"></i> দ্রুত অর্ডার সম্পাদনা
					<?php } ?>
				</div>
				<table class="table table-bordered modern-table dataTable no-footer dtr-inline" id="manageOrderTable">
					<thead>
						<tr>
		...
		?>
			...
			<form class="form-horizontal row" method="POST" action="php_action/editOrder.php" id="editOrderForm">

				<?php $orderId = $_GET['i'];

				$sql = "SELECT orders.order_id, orders.order_date, orders.client_name, orders.client_contact, orders.sub_total, orders.vat, orders.total_amount, orders.discount, orders.grand_total, orders.paid, orders.due, orders.payment_type, orders.payment_status, orders.address,users.full_name,orders.o_feature FROM orders inner join users on
			   users.user_id = orders.s_name
				...
				<div class="col-sm-5">
					<div class="form-group order-group">
						<label for="clientName" class="control-label">ক্রেতার নাম</label>
						<input type="text" class="form-control" id="clientName" name="clientName" placeholder="ক্রেতার নাম" autocomplete="off" value="<?php echo $data[2] ?>" />
					</div>
				</div> <!--/form-group-->
				<div class="col-sm-5 col-sm-offset-1">
					<div class="form-group order-group">
						<label for="clientContact" class="control-label">মোবাইল নাম্বার</label>
						<input type="text" class="form-control" id="clientContact" name="clientContact" placeholder="মোবাইল নাম্বার" autocomplete="off" value="<?php echo $data[3] ?>" />
					</div>
				</div> <!--/form-group-->
				<div class="col-sm-5">
					<div class="form-group order-group">
						<label for="clientaddress" class="control-label">ঠিকানা</label>
						<input type="text" class="form-control" id="clientaddress" name="clientaddress" placeholder="ঠিকানা" autocomplete="off" value="<?php echo $data[13] ?>" />
					</div>
				</div> <!--/form-group-->
				<div class="col-sm-10 col-sm-offset-1">
					<div class="form-group order-group">
						<label for="o_feature" class="control-label">N.B. (Note Well)</label>
						<input type="text" class="form-control" id="o_feature" name="o_feature" value="<?php echo $data[15] ?>" />
					</div>
				</div> <!--/form-group-->
					...
					<tbody>
						...
						?>
							<tr id="row<?php echo $x; ?>" class="table-order <?php echo $arrayNumber; ?>">
								...
								<td>
									<div class="form-group">
										<button class="btn btn-default removeProductRowBtn" type="button" id="removeProductRowBtn" onclick="removeProductRow(<?php echo $x; ?>)" disabled><i class="glyphicon glyphicon-trash"></i></button>
									</div>
								</td>
							</tr>
						<?php
			...
			<div class="row" style="margin-top: 30px;">
				<div class="col-md-6">
					...
					<div class="form-group">
						<label for="discount" class="col-sm-3 control-label">ছাড় (-)</label>
						<div class="col-sm-9">
							<input type="number" class="form-control" id="discount" name="discount" onkeyup="discountFunc()" autocomplete="off" value="<?php echo $data[7] ?>" step="0.01" min="0" />
						</div>
					</div> <!--/form-group-->
					<div class="form-group">
						<label for="grandTotal" class="col-sm-3 control-label">সর্বমোট</label>
				...
				<div class="col-md-6">
					...
					<div class="form-group">
						...
						<div class="col-sm-9">
							<select class="form-control" name="paymentType" id="paymentType">
								<option value="1" <?php if ($data[10] == 2) {
														echo "selected";
													} ?>>টাকা</option>
								<option value="2" <?php if ($data[10] == 1) {
														echo "selected";
													} ?>>চেক</option>
							</select>
						</div>
				...
				<table class="table table-bordered modern-table dataTable no-footer dtr-inline" id="returnProductTable">
					...
					<tfoot>
						<tr style="background-color: #f9f9f9;">
							<td colspan="5" class="text-right"><strong>পূর্বের মোট ফেরতের পরিমাণ:</strong></td>
	...
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-footer removeProductFooter">
				<button type="button" class="btn btn-default" data-dismiss="modal"> <i class="glyphicon glyphicon-remove-sign"></i> বাতিল</button>
				<button type="button" class="btn btn-primary" id="removeOrderBtn" data-loading-text="লোড হচ্ছে..."> <i class="glyphicon glyphicon-ok-sign"></i> সংরক্ষণ করুন</button>
			</div>
		</div><!-- /.modal-content -->
	</div><!-- /.modal-dialog -->
</div><!-- /.modal -->
...
<script>
// Keep hidden rateValue in sync with visible rate input
function syncRateValue(x) {
  ...
  if (rateInput && rateValueInput) {
	rateValueInput.value = rateInput.value;
  }
}
// Update order summary fields when rate changes
function updateOrderSummary() {
	if (typeof discountFunc === 'function') discountFunc();
	if (typeof paidAmount === 'function') paidAmount();
}
</script>
